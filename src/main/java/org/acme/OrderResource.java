...
public class OrderResource {
    ...
    public boolean addOrder(JsonObject body){
        ...
        order.setUserInfo(body.getInteger("userinfo"));
        try {
            client.query("insert into Order1(conditioncode, type1, specialnumber, date1, time1, text1, addExplanation, sendUsers, userInfo)" +
                    "values ('"+order.getConditionCode()+"' , '"+order.getType()+"' , '"+order.getSpecialNumber()+"' , '"+order.getDate()+"' , '"+order.getTime()+"' , '"+order.getText()+"' , '"+order.getAddExplanation()+"' , "+order.getSendUsers()+" , "+order.getUserInfo()+")").execute().await().indefinitely();
